...
class download_goalkeeper_report extends MY_Controller {
		...
		public function index(){
			...
			$data['all_quarter_year'] = $this->dashboard_model->get_all_goalkeeper_quarter_year();
			
			$data['send_email'] = $this->dashboard_model->get_send_mail();
			

			$data['forms_key_detail'] = $this->download_goalkeeper_report_model->get_report_list();
			$data['athlete_list'] =$this->download_goalkeeper_report_model->get_athlete_list();

			//$data['year_by_athlete'] =$this->download_goalkeeper_report_model->get_year_by_athlete();
			//$data['key_perfo_indicat'] =$this->download_goalkeeper_report_model->get_key_perfo_indicat();
			$data['title'] = 'Download Report';

			$this->load->view('admin/includes/_header',$data);
        	$this->load->view('admin/download_goalkeeper_report/download_goalkeeper_report_add',$data);
		...
		public function year_by_athlete(){
			...
			$data_year = $this->download_goalkeeper_report_model->get_year_by_athlete($athlete_id);
		...
		public function select_year(){
			...
			$athlete_id = $_POST['athlete_id'];
			$data_key_per = $this->download_goalkeeper_report_model->get_key_perfo_indicat($year_id,$athlete_id);
		...
		public function report_pdf_download(){
			...
			if($yearly == 2){
				$data['report_name'] = $v_report_name['first_name'].' '.$v_report_name['last_name'].' '.$year_name['year'].' Goalkeeper Yearly Report';
				$data['report_type'] = 3;
				$data['report_heading'] = $year_name['year'].' Yearly Report';

			}else if($first_half == 2){
				$data['report_name'] = $v_report_name['first_name'].' '.$v_report_name['last_name'].' '.$year_name['year'].' Goalkeeper First Half Yearly Report';
				$data['report_type'] = 1;
				$data['report_heading'] = $year_name['year'].' First Half Yearly Report';

			}else if($second_half == 2){
				$data['report_name'] = $v_report_name['first_name'].' '.$v_report_name['last_name'].' '.$year_name['year'].' Goalkeeper Second Half Yearly Report';
				$data['report_type'] = 2;
				$data['report_heading'] = $year_name['year'].' Second Half Yearly Report';
				
			}else{
				$data['report_name'] = $v_report_name['first_name'].' '.$v_report_name['last_name'].' '.$year_name['year'].' Goalkeeper '.$v_report_name['name'];
				$data['report_type'] = 0;
