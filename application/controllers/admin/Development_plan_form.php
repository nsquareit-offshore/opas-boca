...
class Development_plan_form extends MY_Controller {
		...
		public function form_v_json()
		{
		    ...
		    $ids = $this->development_plan_model
		                ->get_if_development_plan_forms_val(
		                    $key_per,
		                    $id_athle,
		                    $id_year
		                );
		    ...
		    if ($id_athle) {
		        $data['athlet_details'] =
		            $this->athlete_model->get_athlete_by_id($id_athle);
		    }
		    ...
		    if ($ids) {
		        ...
		        $data['debug_forms_id'] = $id;
		        $data['development_plan_form_d_id'] = $id;
		        $data['is_active_form'] = $ids['is_active'];

		        // Get ALL Technical Skill records
		        $technical_overview =
		            $this->development_plan_model
		                 ->get_development_plan_overview_repo_id($id);

		        $data['techical_overview'] = $technical_overview;
		...
		public function index(){
			...
			// Delete existing rows when updating
			if (!empty($forms_id)) {
			    $this->development_plan_model
			         ->delete_development_plan_overview($forms_id);
			}

			// Insert all current rows
			if (!empty($development_area_1)) {

			    foreach ($development_area_1 as $key => $area1) {

			        $development_plan_overview = array(
			            'forms_id' => $forms_id,
			            'development_area_1' => $area1,
			            'development_area_2' => isset($development_area_2[$key])
			                ? $development_area_2[$key]
			                : '',
			            'individual_development_plan' => isset($individual_development_plan[$key])
			                ? $individual_development_plan[$key]
			                : '',
			            'remark' => isset($remark[$key])
			                ? $remark[$key]
			                : '',
			        ...
			        $this->development_plan_model
			             ->add_development_plan_overview($development_plan_overview);
			    }
			}
