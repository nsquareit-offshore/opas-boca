<?php
	class download_academic_report_model extends CI_Model{
		...
		public function get_report_list(){
			$this->db->select('
					...
					ci_academic_forms.key_performance_indicator,
	    	...
	    	$this->db->from('ci_academic_forms');

	    	$this->db->join('ci_key_performance', 'ci_key_performance.id = ci_academic_forms.key_performance_indicator ', 'Left');
	    	$this->db->join('ci_athlete', 'ci_athlete.id = ci_academic_forms.athlete_id ', 'Left');
	    	$this->db->join('ci_quarter_years', 'ci_quarter_years.id = ci_academic_forms.quarter_year_id ', 'Left');
		...
		public function get_athlete_list(){			
			...
			// }
		    	...
		    	$this->db->join('ci_academic_forms', 'ci_academic_forms.athlete_id = ci_athlete.id','LEFT');
		...
		public function get_year_by_athlete($athlet_id){
			$this->db->select('
		    	...
		    	$this->db->where('ci_academic_forms.is_active' , 1);
		    	$this->db->where('ci_academic_forms.athlete_id' , $athlet_id);
		    	$query = $this->db->get();					 
				return $query->result_array();
		}
		public function get_key_perfo_indicat($year_id,$athlete_id){
	    	...
	    	$this->db->from('ci_academic_forms');	    	
	    	$this->db->where('ci_academic_forms.is_active' , 1);
	    	$this->db->where('ci_academic_forms.athlete_id' , $athlete_id);
	    	$this->db->where('ci_academic_forms.quarter_year_id' , $year_id);
	    	$query = $this->db->get();					 
			return $query->result_array();
		}
		...
		public function get_report_name($id){
	    	...
	    	$this->db->join('ci_key_performance', 'ci_key_performance.id = ci_academic_forms.key_performance_indicator ', 'Left');
	    	$this->db->join('ci_athlete', 'ci_athlete.id = ci_academic_forms.athlete_id ', 'Left');
	    	$this->db->where('ci_academic_forms.id' , $id);
	    	$query = $this->db->get();					 
		...
		public function get_techical_overview_repo($id){
			...
			$this->db->select('*');
	    	$this->db->from('ci_academic_techical_overview');
		...
		public function get_techical_overview_repo_id($id){
			$query = $this->db->get_where('ci_academic_techical_overview', array('forms_id' => $id));
